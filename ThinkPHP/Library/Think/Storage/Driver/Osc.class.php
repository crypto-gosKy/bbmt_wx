...
class Osc extends Storage {
    ...
    private function getKv() {
        static $kv;
        if (!$kv) {
            $kv = new MemcachedResource('BMTKV');
            $kv>setOption(Memcached::OPT_COMPRESSION, false);
            $kv->setOption(Memcached::OPT_BINARY_PROTOCOL, true);
            if (!$kv)
                E('您没有初始化KVDB，请在OSC管理平台初始化KVDB服务');
        }
        return $kv;
    }
    ...
    public function put($filename, $content, $type = '') {
        switch (strtolower($type)) {
            ...
            default:
                $content = time() . $content;
                if (!$kv->set($filename, $content, MEMCACHE_COMPRESSED, 0)) {
